 ...
 * Never warp.cli / server / webui / updates.source (source is file/env only —
 * session fork pivots would let a LAN client aim AppImage apply at an attacker repo).
 */
const SESSION_ALLOWLIST = {
  ui: new Set(["locale", "theme", "openBrowser"]),
  updates: new Set(["channel", "checkOnStartup"])
};
